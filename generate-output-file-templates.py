#!/usr/bin/env python
#
# Prepare templates for output files from offline radiative transfer
# calculations suitable for publishing on the Earth System Grid
#
...
#
def copy_var(nc_in, nc_out, name, newname=None):
    if newname is None:
        newname = name
    nc_out.createVariable(newname, nc_in.variables[name].dtype,
                          nc_in.variables[name].dimensions)
...
parser = argparse.ArgumentParser(
    description='Create CMIP6/ESGF-compliant output files for RFMIP-IRF.')
parser.add_argument('--source_id', type=str, default="LBLRTM-12-8",
                    help='Source ID, must match CMIP Controlled Vocabulary at '
                         'https://github.com/WCRP-CMIP/CMIP6_CVs/blob/master'
                         '/CMIP6_source_id.json')
parser.add_argument('--forcing_index', type=int, default=1,
                    help='Forcing index (1 = all available greenhouse gases; '
                         '2 =  CO2, CH4, N2O, CFC12, CFC11eq; '
                         '3 = CO2, CH4, N2O, CFC12eq, HFC-134eq)')
parser.add_argument('--physics_index', type=int, default=1,
...
#
url = urlopen("https://raw.githubusercontent.com/PCMDI/cmip6-cmor-tables"
              "/master/Tables/CMIP6_CV.json")
...
if args.source_id in cmip6['CV']['source_id'].keys():
    ...
    print("source_id {} is not in CMIP6 Controlled Vocabulary".format(
        args.source_id))
    sys.exit(1)

if (forcing_index < 1) or (forcing_index > 3):
    print('forcing_index must be 1, 2, or 3 (1 = all available greenhouse '
          'gases; 2 =  CO2, CH4, N2O, CFC12, CFC11eq;  3 = CO2, CH4, N2O, '
          'CFC12eq, HFC-134eq)')
    sys.exit(1)
...
model_attrs = {
    ...
    "further_info_url": "https://furtherinfo.es-doc.org/CMIP6." +
                        institution_id + "." + source_id + ".rad-irf.none." +
                        variant_label,
    "forcing_index": np.int32(forcing_index),
    "license": "CMIP6 model data produced by " + institution_id +
               ...
               "including some limitations, can be found via the further "
               "info_url (recorded as a global attribute in this file) and at "
               "https://pcmdi.llnl.gov/. The data producers and data "
               "providers make no warranty, either express or implied, "
               "including, but not limited to, warranties of merchantability "
               "and fitness for a particular purpose. All liabilities arising "
               "from the supply of the information (including any liability "
               "arising in negligence) are excluded to the fullest extent "
               "permitted by law."}
...
drs_attrs = {
    ...
    "table_id": "Efx",
    # per
    # http://clipc-services.ceda.ac.uk/dreq/u/efc0de22-5629-11e6-9079-ac72891c3257.html
    "frequency": "fx",
...
expt_attrs = {
    ...
    "mip_era": "CMIP6",
    "experiment": "offline assessment of radiative transfer parmeterizations "
                  "in clear skies",
    "sub_experiment": "none",
    "product": "model-output",
...
stand_names = ['upwelling_longwave_flux_in_air',
               'upwelling_shortwave_flux_in_air',
               'downwelling_longwave_flux_in_air',
               'downwelling_shortwave_flux_in_air']

for short, std in zip(short_names, stand_names):
    # File name is constructed per
    # https://docs.google.com/document/d/1h0r8RZr_f3-8egBMMh7aqLwy3snpD6_MrDz1q8n5XUk/edit#
    # fixed strings are table_id, experiment_id, grid_label
    out_file_name = short + "_Efx_" + source_id +\
                    "_rad-irf_" + variant_label + "_gn" + ".nc"
    print('Creating ' + out_file_name)
    out_file = Dataset(out_file_name, mode='w')
    ...
    out_file.createDimension('expt', atmos_file.dimensions['expt'].size)
    out_file.createDimension('site', atmos_file.dimensions['site'].size)
    out_file.createDimension('level', atmos_file.dimensions['level'].size)
    copy_var(atmos_file, out_file, 'lat')
    copy_var(atmos_file, out_file, 'lon')
    copy_var(atmos_file, out_file, 'time')
    copy_var(atmos_file, out_file, 'pres_level', 'plev')
    ...
    copy_var(atmos_file, out_file, 'profile_weight')
    out_file.close()
